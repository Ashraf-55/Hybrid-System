// ============================================================
//  نظام إدارة العيادة - الكود الرئيسي
//  التعديلات: Issues #3, #14, #19 + Issues #1 #11 #15
// ============================================================

// ========== إعدادات السحابة (Cloud Sync) ==========
const CLOUD_CONFIG = {
    enabled: false,
    provider: 'firebase',
...
const CloudSyncManager = (() => {
    ...
    function _firebaseEndpoint(key) {
        const base = CLOUD_CONFIG.baseUrl.replace(/\/$/, '');
        const path = `${base}/${CLOUD_CONFIG.clinicId}/${key}.json`;
        return CLOUD_CONFIG.apiKey ? `${path}?auth=${CLOUD_CONFIG.apiKey}` : path;
    }

    async function _firebasePut(key, data) {
        const res = await fetch(_firebaseEndpoint(key), {
            method: 'PUT',
    ...
    async function _firebasePullAll() {
        const base = CLOUD_CONFIG.baseUrl.replace(/\/$/, '');
        const url = `${base}/${CLOUD_CONFIG.clinicId}.json` +
            (CLOUD_CONFIG.apiKey ? `?auth=${CLOUD_CONFIG.apiKey}` : '');
    ...
    async function _supabasePut(key, data) {
        const url = `${CLOUD_CONFIG.supabaseUrl}/rest/v1/${CLOUD_CONFIG.supabaseTable}`;
        const res = await fetch(url, {
            method: 'POST',
            ...
            body: JSON.stringify({
                ...
                clinic_id: CLOUD_CONFIG.clinicId,
    ...
    async function _supabaseGet(key) {
        const url = `${CLOUD_CONFIG.supabaseUrl}/rest/v1/${CLOUD_CONFIG.supabaseTable}` +
            `?key=eq.${key}&clinic_id=eq.${CLOUD_CONFIG.clinicId}&select=value`;
    ...
    async function _supabasePullAll() {
        const url = `${CLOUD_CONFIG.supabaseUrl}/rest/v1/${CLOUD_CONFIG.supabaseTable}` +
            `?clinic_id=eq.${CLOUD_CONFIG.clinicId}&select=key,value`;
        const res = await fetch(url, { headers: _supabaseHeaders() });
        if (!res.ok) return null;
        const rows = await res.json();
        const snapshot = {};
        rows.forEach(r => { try { snapshot[r.key] = JSON.parse(r.value); } catch (_) { } });
        return snapshot;
    }

    async function _putData(key, data) {
        if (CLOUD_CONFIG.provider === 'supabase') return _supabasePut(key, data);
        return _firebasePut(key, data);
    }
    async function _getData(key) {
        if (CLOUD_CONFIG.provider === 'supabase') return _supabaseGet(key);
        return _firebaseGet(key);
    }
    async function _pullAllData() {
        if (CLOUD_CONFIG.provider === 'supabase') return _supabasePullAll();
        return _firebasePullAll();
    }
    ...
    async function _flush() {
        if (!CLOUD_CONFIG.enabled || !_online) return;
        const entries = Object.entries(_writeQueue);
        ...
        for (const [key, data] of entries) {
            try {
                ...
                console.warn(`[CloudSync] Write failed for "${key}":`, err.message);
                _writeQueue[key] = data;
                if (_retryCount < CLOUD_CONFIG.maxRetries) {
                    _retryCount++;
                    setTimeout(_flush, CLOUD_CONFIG.retryDelayMs);
                }
            }
        }
    }
    ...
    function save(key, data) {
        try { localStorage.setItem(`clinic_${key}`, JSON.stringify(data)); } catch (e) { }
        if (CLOUD_CONFIG.enabled) { _writeQueue[key] = data; _scheduleFlush(); }
    }
    ...
    async function load(key, defaultValue) {
        ...
        if (CLOUD_CONFIG.enabled && _online) {
            try {
                const data = await _getData(key);
                if (data !== null && data !== undefined) {
                    try { localStorage.setItem(`clinic_${key}`, JSON.stringify(data)); } catch (_) { }
                    return data;
                }
            } catch (err) { console.warn(`[CloudSync] Load failed for "${key}":`, err.message); }
        }
        return defaultValue;
    }

    async function pullAll() {
        if (!CLOUD_CONFIG.enabled || !_online) return false;
        try {
            const snapshot = await _pullAllData();
            if (!snapshot) return false;
            Object.entries(snapshot).forEach(([key, value]) => {
                try { localStorage.setItem(`clinic_${key}`, JSON.stringify(value)); } catch (e) { }
            });
            return true;
        } catch (err) {
            console.warn('[CloudSync] pullAll failed:', err.message);
            return false;
        }
    }
    ...
    async function init() {
        ...
        if (pulled) console.info(`[CloudSync] ✅ Data hydrated from cloud (${CLOUD_CONFIG.provider}).`);
        setInterval(() => { if (_online) _flush(); }, CLOUD_CONFIG.syncIntervalMs);
    }

    return { save, load, pullAll, init };
})();
...
let currentUser = loadFromStorage('clinic_current_user', null);
...
function showLoginPage() {
    ...
    if (lp) { lp.style.display = 'flex'; lp.classList.add('login-fullpage'); }
}
...
function checkAuth() {
    if (currentUser) { showAppInterface(); showSection('dashboard'); }
    else { showLoginPage(); }
}
...
function saveAllData(forceFullSave = false) {
    ...
    try { localStorage.setItem('clinic_current_user', JSON.stringify(currentUser)); } catch (e) { }
}

function initializeData() {
    patients = patients.map(p => ({
        id: p.id || generateId(), created_at: p.created_at || getTodayDate(),
        chronic: p.chronic || 'لا يوجد', birthday: p.birthday || null,
        city: p.city || 'غير محدد', ...p
    }));
    visits = visits.map(v => ({
        id: v.id || generateId(), visit_date: v.visit_date || getTodayDate(),
        total_fees: v.total_fees || 0, diagnosis: v.diagnosis || 'لم يسجل',
        prescription: v.prescription || 'لم تسجل', status: v.status || 'completed',
        doctorEarning: v.doctorEarning || 0, ...v
    }));
    appointments = appointments.map(a => ({ id: a.id || generateId(), status: a.status || 'scheduled', ...a }));
    pharmacy = pharmacy.map(d => ({ id: d.id || generateId(), minStock: d.minStock || 5, ...d }));
    inventory = inventory.map(i => ({
        id: i.id || generateId(), warehouseId: i.warehouseId || 'wh-main',
        minStock: i.minStock || i.min_limit || 0,
        expiryDate: i.expiryDate || null, ...i
    }));

    if (!warehouses.length) {
        warehouses.push({
            id: 'wh-main', name: 'المخزن الرئيسي', type: 'main',
            location: '', manager: '', notes: '', createdAt: new Date().toISOString()
        });
    }
    if (!employees.length) {
        employees.push({
            id: 'emp-admin', username: 'admin', password: 'admin123',
            fullName: 'مدير النظام', role: 'admin', email: 'admin@clinic.com',
            phone: '0123456789', position: 'مدير عام', department: 'الإدارة',
            isActive: true, hireDate: getTodayDate(), salary: 5000, shift: 'morning',
...
function getCurrentTime() { return new Date().toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' }); }
function formatCurrency(amount) { return Number(amount || 0).toLocaleString() + ' ج.م'; }
...
function getCategoryName(cat) {
    return ({
        rent: 'إيجار', salaries: 'مرتبات', electricity: 'كهرباء', water: 'مياه',
        maintenance: 'صيانة', other: 'أخرى'
    })[cat] || cat;
}

function getPaymentMethod(method) {
    return ({ cash: 'كاش', card: 'بطاقة', bank: 'تحويل بنكي' })[method] || method;
}
...
function showSection(id) {
    if (currentUser && typeof hasPermission === 'function') {
        ...
        if (role === 'doctor' && doctorBlocked.includes(id)) {
            showToast('⛔ ليس لديك صلاحية لعرض هذا القسم', 'error'); return;
        }
...
function loadSectionData(id) {
    const loaders = {
        ...
        'appointments': () => {
            loadAppointments(); populatePatientSelect();
        ...
        'insurance-companies': () => { loadInsuranceCompanies(); hidePackagesSection(); },
        ...
        'stock-ledger': () => { populateLedgerFilter(); loadStockLedger(); },
        'labs': loadLabs,
        'lab-requests': () => loadLabRequests('all'),
        'lab-templates': loadLabTemplates,
        'employees': loadEmployees,
        'attendance': loadAttendance,
        'tasks': () => { populateTaskAssignee(); loadTasks('all'); },
...
function handleRouteChange() {
    ...
    else if (currentUser) showSection('dashboard');
    else showLoginPage();
}

function goBack() {
    if (navigationHistory.length > 0) showSection(navigationHistory.pop());
    else { showSection('dashboard'); showToast('لا يوجد صفحات سابقة', 'info'); }
}
...
function updateDashboard() {
    ...
    if (badge) {
        if (unreadCount > 0) { badge.style.display = 'inline'; badge.innerText = unreadCount > 99 ? '99+' : unreadCount; }
...
function previewFile(input, previewId) {
    ...
    if (!preview || !input.files || !input.files[0]) { if (preview) preview.innerHTML = ''; return; }
    const file = input.files[0];
    if (file.size > 10 * 1024 * 1024) { showToast('الملف كبير جداً (الحد الأقصى 10 ميجابايت)', 'warning'); input.value = ''; preview.innerHTML = ''; return; }
...
function fileToBase64(file) {
    return new Promise((resolve, reject) => {
        ...
        if (file.size > 10 * 1024 * 1024) { showToast('الملف كبير جداً', 'warning'); reject(new Error('File too large')); return; }
        const reader = new FileReader();
        reader.readAsDataURL(file);
...
function loadPatients() {
    ...
    tbody.innerHTML = !patients.length
        ? ' why<td colspan="5" style="text-align:center;padding:30px;">👥 لا يوجد مرضى</td> </tr>'
        : patients.map(p => `
             <tr>
                 <td>${p.name || '---'}</td>
                 <td>${p.phone || '---'}</td>
...
function clearPatientForm() {
    ['patientName', 'patientPhone', 'patientNationalId', 'patientAge',
        'patientChronic', 'patientBirthday', 'patientCity'].forEach(id => {
            const el = document.getElementById(id); if (el) el.value = '';
        });
    const g = document.getElementById('patientGender'); if (g) g.value = 'ذكر';
}
...
function saveNewPatient() {
    ...
    if (!name) { showToast('الرجاء إدخال اسم المريض', 'warning'); return; }
    if (!age) { showToast('الرجاء إدخال عمر المريض', 'warning'); return; }
    const ageNum = parseInt(age);
    if (isNaN(ageNum) || ageNum <= 0 || ageNum > 150) { showToast('الرجاء إدخال عمر صحيح (1-150)', 'warning'); return; }
...
function showPatientQR(patientId) {
    ...
    if (!qr) { showToast('لا يمكن توليد QR Code', 'warning'); return; }
...
function deletePatient(id) {
    ...
    saveAllData(); loadPatients(); updateDashboard();
...
function saveVisit() {
    if (!currentPatientId) { showToast('لا يوجد مريض محدد', 'error'); closeModal(); return; }
    ...
    if (!diagnosisInput || !prescriptionInput || !totalFeesInput) {
        showToast('خطأ في النموذج', 'error'); return;
    }
    ...
    Promise.all([fileToBase64(scanFile), fileToBase64(labFile)]).then(([scanData, labData]) => {
        const newVisit = {
            ...
            scan_desc: scanName, lab_desc: labName,
            scan_file: scanData, lab_file: labData,
            scan_name: scanFile?.name || null, lab_name: labFile?.name || null,
            scan_type: scanFile?.type || null, lab_type: labFile?.type || null,
            scan_size: scanFile?.size || null, lab_size: labFile?.size || null,
        ...
        [diagnosisInput, prescriptionInput, scanNameInput, labNameInput, totalFeesInput, scanFileInput, labFileInput]
            .forEach(el => { if (el) el.value = ''; });
        if (doctorSelect) doctorSelect.value = '';
        ['scan_preview', 'lab_preview'].forEach(id => { const el = document.getElementById(id); if (el) el.innerHTML = ''; });

        closeModal(); loadPatients(); updateDashboard();
...
function viewHistory(patientId, patientName) {
    ...
    if (!patientVisits.length) {
        ...
        [...patientVisits].sort((a, b) => new Date(b.visit_date) - new Date(a.visit_date)).forEach(v => {
            ...
            if (v.scan_desc && q('.v-scan-desc')) { q('.v-scan-desc').innerText = v.scan_desc; if (q('.no-scan')) q('.no-scan').style.display = 'none'; }
            if (v.lab_desc && q('.v-lab-desc')) { q('.v-lab-desc').innerText = v.lab_desc; if (q('.no-lab')) q('.no-lab').style.display = 'none'; }
...
function loadInventory() {
    ...
    tbody.innerHTML = !inventory.length
        ? '<tr><td colspan="6" style="text-align:center;padding:30px;">📦 لا توجد أصناف\r\n      </td>\r\n    </tr>' : inventory.map(item => {
...
function addNewInventoryItem() {
    ...
    if (!name || isNaN(qty) || qty <= 0) { showToast('بيانات المخزن ناقصة', 'warning'); return; }
    inventory.push({
        id: generateId(), product_name: name, quantity: qty, price,
        min_limit: minStock,
        minStock,
        expiryDate,
        warehouseId: 'wh-main'
    ...
    markDirty('inventory');
    saveAllData(); loadInventory(); updateDashboard();
    showToast('✅ تمت إضافة الصنف');

    checkInventoryAlerts();

    ['inv_name', 'inv_qty', 'inv_price', 'inv_limit', 'inv_min_stock', 'inv_expiry']
...
function checkPharmacyAlerts() {
    ...
    pharmacy.forEach(drug => {
        ...
        if (drug.expiry) {
            const exp = new Date(drug.expiry); exp.setHours(0, 0, 0, 0);
            const daysLeft = Math.round((exp - today) / 86400000);
